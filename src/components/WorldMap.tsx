...
export default function WorldMap({countries,flows,onSelect}:{countries:CountryState[];flows:Flow[];onSelect:(c:CountryState)=>void}){
  const wrap=useRef<HTMLDivElement>(null),globe=useRef<GlobeMethods|undefined>(undefined),[width,setWidth]=useState(900),[shapes,setShapes]=useState<CountryShape[]>([]),[hovered,setHovered]=useState<string|null>(null),[selected,setSelected]=useState<string|null>(null),[reducedMotion,setReducedMotion]=useState(false);
  ...
  return <div ref={wrap} className="globe-wrap glass">
    <Globe ref={globe} onGlobeReady={()=>globe.current?.pointOfView({lat:12,lng:-18,altitude:1.65})} width={width} height={570} backgroundColor="#020609" backgroundImageUrl="/earth/night-sky.png" globeImageUrl="/earth/earth-blue-marble.jpg" bumpImageUrl="/earth/earth-topology.png" showAtmosphere atmosphereColor="#72c7ff" atmosphereAltitude={.16}
      ...
      arcsData={arcs} arcStartLat="startLat" arcStartLng="startLng" arcEndLat="endLat" arcEndLng="endLng" arcAltitude={.1} arcColor={()=>["rgba(125,232,255,.08)","rgba(125,232,255,.62)"]} arcStroke={(d:object)=>.12+arc(d).strength*.32} arcDashLength={.38} arcDashGap={1.4} arcDashAnimateTime={reducedMotion?0:5200}/>
