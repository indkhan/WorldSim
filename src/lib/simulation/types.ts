...
export type CohortState={id:string;kind:ActorKind;income:Income;urban:boolean;sector:string;weight:number;share:number;resilience:number;exposure:number;pressure:number;action:string;confidence:number;rationale:string;decisionSource:"rules"|"openrouter";actionEffect:number};
...
export type SimulationResult={id?:string;scenario:ScenarioInput;frames:SimulationFrame[];methodology:string;analysis?:string|null;providerMode:"deterministic"|"openrouter";createdAt?:string;warnings?:string[]};
